...
const resolveBillingStatus = ({
  ...
  const safePrevDue =
    previousDueRemaining !== null
      ? Number(previousDueRemaining || 0)
      : Number(previousDue || 0);
  const safePrevAdvance =
    carryForwardNext !== null && carryForwardNext > 0
      ? Number(carryForwardNext || 0)
      : Number(previousAdvance || 0);

  const totalRequired = Math.max(0, safeBill + safePrevDue - safePrevAdvance);
  const calcEndingBalance =
    endingBalance !== null
      ? Number(endingBalance || 0)
      : safePrevAdvance - safePrevDue + safePaid - safeBill;
  ...
  const isPastMonth =
    hasPeriodContext &&
    (safeYear < currentYear || (isCurrentYear && safeMonth < currentMonth));

  const monthEndBoundary = hasPeriodContext
    ? new Date(safeYear, safeMonth, 0, 23, 59, 59, 999)
    : null;
  const isMonthEnded =
    isPastMonth ||
    (isCurrentMonth && now.getTime() > monthEndBoundary?.getTime());
...
export const buildYearlyCustomerReportSummary = ({
  ...
  const safeYear = Number(year);
  const customerPayments = getActivePayments(payments || []).filter((payment) =>
    matchesPaymentToUser(payment, user),
  );
  console.log("Selected User:", user);
  console.log("Customer Payments:", customerPayments);
  console.log("Total Customer Payments:", customerPayments.length);
  const joinDate = parseDateValue(
    user?.joinDate ||
      user?.joinedAt ||
      user?.memberSince ||
  ...
  const openingRow = buildMonthlySheetLedgerRow({
    user,
    payments: [],
    history: customerPayments.filter((payment) => {
      const { year } = getPaymentMonthYear(payment);
      return Number(year) < safeYear;
    }),
    month: 1,
    year: safeYear,
    isActiveForPeriod: isUserActiveForPeriod,
  ...
  const months = Array.from({ length: 12 }, (_, index) => {
    ...
    const monthPayments = (customerPayments || []).filter((payment) => {
      const { month: paymentMonth, year: paymentYear } =
        getPaymentMonthYear(payment);
      return (
        matchesPaymentToUser(payment, user) &&
  ...
  const allActiveYearPayments = customerPayments
    .filter((payment) => {
      const { year: paymentYear } = getPaymentMonthYear(payment);
