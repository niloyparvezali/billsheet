...
const defaultSmsTemplate = 'Dear {name}, your monthly bill is {bill}. Please pay by {duedate}. Thank you.'
const createSms = (template, { name, bill, dueDate }) => template.replaceAll('{name}', name || 'Customer').replaceAll('{bill}', money(bill)).replaceAll('{duedate}', dueDate)
...
export default function MonthlySheet() {
  ...
  const sendSms = async user => {
    ...
    if (!/^\+?\d+$/.test(phone)) return toast.error('Enter a valid phone number before sending an SMS')
    try {
